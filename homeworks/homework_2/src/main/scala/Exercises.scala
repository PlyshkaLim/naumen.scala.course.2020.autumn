object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        for {i <- 2 to number
             if (is_simple(i) == true && number % i == 0)
             } yield i
    }

    def is_simple(a: Int): Boolean = {
        var flag = true
        for (i <- 2 to a - 1)
             if (a % i == 0) {
                 flag = false
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.mapValues(i => BallWeight(i._1, i._2)).toList.sortBy(_._2).map(_._1)
    }

    def BallWeight(a: (Int, Double)): Double = (4 / 3 * java.lang.Math.PI * Math.pow(a._1, 3)) * a._2
